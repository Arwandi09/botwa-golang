...
// Owner dianggap valid kalau: pesan dikirim dari akun bot sendiri
// (IsFromMe), ATAU pengirimnya ada di daftar OwnerNumbers di config.go.
func isOwner(m *events.Message) bool {
	if m.Info.IsFromMe {
		return true
	}
	for _, n := range ownerNumbers {
		if m.Info.Sender.User == n {
			return true
		}
	}
	return false
...
func jadibotMessageHandler(client *whatsmeow.Client) func(interface{}) {
	return func(evt interface{}) {
		...
		case *events.Message:
			...
			// CEK VIEW ONCE VIA REPLY: hanya aktif kalau pengguna sendiri
// me-reply pesan media sekali-lihat. Sama seperti di handler.go
// bot utama, supaya perilaku clone jadibot konsisten.
if err := RevealViewOnce(client, m); err != nil {
    return
}
...
func StartJadibotSession(targetNumber string, notifyClient *whatsmeow.Client, notifyChat types.JID) {
	...
	if client.Store.ID == nil {
		code, err := client.PairPhone(ctx, targetNumber, true, whatsmeow.PairClientChrome, "Chrome (Linux)", pairingCode)
		if err != nil {
			if notifyClient != nil {
