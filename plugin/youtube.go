...
func ytAudio(client *whatsmeow.Client, m *events.Message, args []string) {
	...
	// --no-part: yt-dlp menulis langsung ke nama file akhir (tanpa file sementara .part),
	//   jadi tidak ada langkah "rename .part -> final" yang rawan gagal di penyimpanan Android/Termux.
	// --retries / --fragment-retries: otomatis coba ulang kalau ada hiccup jaringan sesaat.
	cmd := exec.Command(
		"yt-dlp",
		"-x",
		"--audio-format", "mp3",
	...
	// Cek dulu apakah file hasil akhirnya benar-benar ada. yt-dlp kadang sempat
	// menampilkan ERROR sesaat (mis. rename gagal) tapi tetap berhasil
	// menyelesaikan proses & exit code-nya jadi tidak konsisten dengan hasil
	// aslinya — jadi jangan langsung vonis gagal hanya dari err != nil.
	if _, statErr := os.Stat(fileTarget); statErr != nil {
		if err != nil {
			reactError(client, m)
...
func ytVideo(client *whatsmeow.Client, m *events.Message, args []string) {
	...
	// --no-part, --retries, --fragment-retries: sama seperti di ytAudio, mengurangi
	...
	// Sama seperti ytAudio: cek keberadaan file dulu sebelum vonis gagal,
	// karena exit code yt-dlp kadang tidak konsisten meski hasil akhirnya jadi.
	if _, statErr := os.Stat(fileTarget); statErr != nil {
		if err != nil {
			reactError(client, m)
