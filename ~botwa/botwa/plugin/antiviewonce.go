...
// RevealViewOnce otomatis mengecek pesan yang SEDANG DI-REPLY oleh pengguna.
// Jika pesan yang di-reply adalah media sekali-lihat (view-once), bot langsung
// mengirim ulang mediannya ke chat yang sama — tanpa perlu command/kata kunci apa pun,
// cukup reply pesannya. Tidak ada forwarding diam-diam ke nomor lain: hanya jalan saat
// pengguna sendiri membalas pesan tersebut.
func RevealViewOnce(client *whatsmeow.Client, m *events.Message) bool {
	...
	if !isViewOnce || (imgMsg == nil && vidMsg == nil && audMsg == nil && docMsg == nil) {
		// Bukan reply ke media sekali-lihat -> biarkan pesan diproses handler lain seperti biasa
		return false
	}

	// Target = chat yang sama tempat command dijalankan (bukan nomor tersembunyi)
	targetJID := m.Info.Chat

	sendFailNotice := func(action string, e error) {
		fmt.Printf("[RevealViewOnce] %s: %v\n", action, e)
		client.SendMessage(ctx, targetJID, &proto.Message{
			Conversation: proto.String(fmt.Sprintf("Gagal membuka media sekali-lihat (%s).", action)),
		})
	}
...
// getQuotedMessage mengambil pesan yang di-quote (di-reply) dari ContextInfo,
// jika ada, baik dari ExtendedTextMessage maupun tipe pesan lain yang punya ContextInfo.
func getQuotedMessage(msg *proto.Message) *proto.Message {
	...
	// Beberapa client mengirim reply pada tipe pesan lain (mis. gambar dengan caption
	// yang juga berupa reply) — ContextInfo bisa ada di situ juga.
