...
// ============================================================================
// PATCHED VERSION — menambahkan dukungan CUSTOM PAIRING CODE
// (mirip fitur `pairing.code` di neoxr-bot / Baileys)
//
// Perubahan dari versi asli tulir/whatsmeow:
//   1. generateCompanionEphemeralKey() sekarang menerima parameter customCode.
//      Jika customCode diisi, dipakai langsung sebagai kode pairing
//      (bukan hasil random.Bytes(5)).
//   2. PairPhone() sekarang menerima parameter variadic customCode (opsional),
//      jadi tetap backward-compatible dengan pemanggilan yang sudah ada.
//   3. Validasi: custom code WAJIB persis 8 karakter (huruf/angka), sesuai
//      panjang kode pairing WhatsApp yang asli (format XXXX-XXXX).
// ============================================================================
...
// ErrCustomPairingCodeInvalidLength is returned when a custom pairing code
// passed to PairPhone is not exactly 8 characters long.
var ErrCustomPairingCodeInvalidLength = fmt.Errorf("custom pairing code must be exactly 8 characters")
...
// generateCompanionEphemeralKey creates the ephemeral keypair used for pairing.
// If customCode is non-empty, it is used as the pairing code instead of a
// randomly generated one. customCode must be exactly 8 characters.
func generateCompanionEphemeralKey(customCode string) (ephemeralKeyPair *keys.KeyPair, ephemeralKey []byte, encodedLinkingCode string, err error) {
	ephemeralKeyPair = keys.NewKeyPair()
	salt := random.Bytes(32)
	iv := random.Bytes(16)

	if customCode != "" {
		normalized := strings.ToUpper(strings.ReplaceAll(customCode, "-", ""))
		if len(normalized) != 8 {
			return nil, nil, "", ErrCustomPairingCodeInvalidLength
		}
		encodedLinkingCode = normalized
	} else {
...
//
// customCode (optional, variadic): pass an 8-character string (letters/digits, dash optional)
// to use YOUR OWN pairing code instead of a randomly generated one — e.g. PairPhone(ctx, phone,
// true, PairClientChrome, "Chrome (Linux)", "NEOXRBOT"). Leave empty to keep default random behavior.
//
// See https://faq.whatsapp.com/1324084875126592 for more info
func (cli *Client) PairPhone(ctx context.Context, phone string, showPushNotification bool, clientType PairClientType, clientDisplayName string, customCode ...string) (string, error) {
	if cli == nil {
	...
	ephemeralKeyPair, ephemeralKey, encodedLinkingCode, err := generateCompanionEphemeralKey(custom)
	if err != nil {
		return "", err
	}
